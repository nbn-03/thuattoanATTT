def lastOccurrence(p,T):# Hàm tìm vị trí của các phần tử của T trong p nếu có nhiều vị trí thì lấy vị trí ở cuối
    ...
    for i in range(len(x)):#Duyệt các phần tử trong x
        ...
        for j in range(len(p)-1,-1,-1): # Duyệt các phàn tử trong p
            if p[j]==x[i]: # Nếu tìm thấy x[i] trong p với vt được gán là vị trí có chỉ số cao nhất của x[i] trong p
                vt = j
                break
        f.append(vt)
    return x,f
